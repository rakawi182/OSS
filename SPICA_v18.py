...
# ============================================================================
# SMART WARA PARSER - VERSI MINIMALIS TANPA DEPENDENSI EKSTERNAL
# ============================================================================

class SmartWaraParser:
    """
    Parser cerdas untuk wara tidak lengkap.
    Hanya menggunakan data dari ΩConstants dan MechanicalEngine.
    Tidak ada tambahan normalisasi, tidak ada error correction.
    """
    
    def __init__(self, mech_engine):
        self.mech = mech_engine
        self.norm = NormalizationEngine()
        self.const = ΩConstants
        
        # Gunakan mapping yang sudah ada di MechanicalEngine
        self.triple_to_index = mech_engine.wara_triple_to_index
        self.triple_to_wuku = mech_engine.wara_triple_to_wuku
        self.wuku_all_triples = mech_engine.wuku_all_triples
        
        # Pre-compute untuk performa (ringan)
        self._build_component_sets()
    
    def _build_component_sets(self):
        """Bangun set untuk pencarian cepat"""
        self.sadwara_set = set(self.norm.normalize(s) for s in self.const.SADWARA_STANDARD)
        self.pancawara_set = set(self.norm.normalize(p) for p in self.const.PANCAWARA_STANDARD)
        self.saptawara_set = set(self.norm.normalize(s) for s in self.const.SAPTAWARA_STANDARD)
        self.wuku_set = set(self.norm.normalize(w) for w in self.const.WUKU_NAMES_STANDARD)
    
    def identify_component_type(self, component: str) -> str:
        """Identifikasi tipe komponen: sadwara, pancawara, atau saptawara"""
        norm_component = self.norm.normalize(component)
        
        if norm_component in self.sadwara_set:
            return "sadwara"
        elif norm_component in self.pancawara_set:
            return "pancawara"
        elif norm_component in self.saptawara_set:
            return "saptawara"
        else:
            return "unknown"
    
    def parse_wara_intelligent(self, wara_string: str, wuku: str = None) -> Dict:
        """
        Parsing cerdas untuk semua format wara.
        Mengembalikan semua kemungkinan triple yang valid.
        """
        if not wara_string:
            return {"error": "wara_string kosong"}
        
        parts = [p.strip() for p in wara_string.split('-') if p.strip()]
        
        # **KASUS 1: FULL TRIPLE (3 bagian)**
        if len(parts) == 3:
            return self._parse_full_triple(parts, wuku)
        
        # **KASUS 2: PARTIAL (2 bagian)**
        elif len(parts) == 2:
            return self._parse_partial_two(parts, wuku)
        
        # **KASUS 3: SINGLE (1 bagian)**
        elif len(parts) == 1:
            return self._parse_single(parts[0], wuku)
        
        else:
            return {"error": "Format tidak dikenali"}
    
    def _parse_full_triple(self, parts: List[str], wuku: str) -> Dict:
        """Parse format lengkap: sadwara-pancawara-saptawara"""
        sad, panca, sapta = parts
        ...
        if triple_key in self.triple_to_index:
            wuku_calculated = self.triple_to_wuku.get(triple_key)
            
            # Validasi wuku jika diberikan
            if wuku:
                wuku_norm = self.norm.normalize(wuku)
                if wuku_calculated != wuku_norm:
                    return {
                        "error": f"Triple tidak cocok dengan wuku {wuku}",
                        "expected_wuku": wuku_calculated
                    }
            
            return {
                "type": "full",
                "sadwara": sad_norm,
                "pancawara": panca_norm,
                "saptawara": sapta_norm,
                "wuku": wuku_calculated,
                "confidence": 1.0,
                "all_possibilities": [{
                    "sadwara": sad_norm,
                    "pancawara": panca_norm,
        ...
        return {"error": "Triple tidak valid dalam sistem"}
    
    def _parse_partial_two(self, parts: List[str], wuku: str) -> Dict:
        """
        Parsing 2 parts dengan kecerdasan tinggi.
        Menganalisis semua kemungkinan interpretasi.
        """
        part1, part2 = parts
        part1_norm = self.norm.normalize(part1)
        part2_norm = self.norm.normalize(part2)
        wuku_norm = self.norm.normalize(wuku) if wuku else None
        
        # Identifikasi tipe setiap part
        type1 = self.identify_component_type(part1)
        type2 = self.identify_component_type(part2)
        
        # **ALGORITMA CERDAS: Analisis semua kemungkinan interpretasi**
        all_possibilities = []
        
        # **INTERPRETASI 1: sadwara-pancawara** (jika part1 sadwara, part2 pancawara)
        if type1 == "sadwara" and type2 == "pancawara":
            possibilities = self._find_by_sad_panca(part1_norm, part2_norm, wuku_norm)
            all_possibilities.extend(possibilities)
        
        # **INTERPRETASI 2: pancawara-sadwara** (jika part1 pancawara, part2 sadwara)
        elif type1 == "pancawara" and type2 == "sadwara":
            possibilities = self._find_by_sad_panca(part2_norm, part1_norm, wuku_norm)
            all_possibilities.extend(possibilities)
        
        # **INTERPRETASI 3: saptawara-pancawara** (paling umum untuk format seperti "Jumat-Wage")
        if type1 == "saptawara" and type2 == "pancawara":
            possibilities = self._find_by_sapta_panca(part1_norm, part2_norm, wuku_norm)
            all_possibilities.extend(possibilities)
        
        # **INTERPRETASI 4: pancawara-saptawara**
        elif type1 == "pancawara" and type2 == "saptawara":
            possibilities = self._find_by_sapta_panca(part2_norm, part1_norm, wuku_norm)
            all_possibilities.extend(possibilities)
        
        # **INTERPRETASI 5: sadwara-saptawara** (jarang, tapi mungkin)
        if type1 == "sadwara" and type2 == "saptawara":
            possibilities = self._find_by_sad_sapta(part1_norm, part2_norm, wuku_norm)
            all_possibilities.extend(possibilities)
        
        # **INTERPRETASI 6: saptawara-sadwara**
        elif type1 == "saptawara" and type2 == "sadwara":
            possibilities = self._find_by_sad_sapta(part2_norm, part1_norm, wuku_norm)
            all_possibilities.extend(possibilities)
        
        # **INTERPRETASI 7: Jika tidak bisa diidentifikasi, coba semua kombinasi**
        if type1 == "unknown" or type2 == "unknown":
            possibilities = self._find_by_any_two(part1_norm, part2_norm, wuku_norm)
            all_possibilities.extend(possibilities)
        ...
        for poss in all_possibilities:
            key = (poss["sadwara"], poss["pancawara"], poss["saptawara"])
            if key not in seen:
                seen.add(key)
                unique_possibilities.append(poss)
        
        if not unique_possibilities:
            return {
                "error": "Tidak ditemukan kombinasi yang valid",
                "debug_info": {
                    "part1": part1,
                    "part2": part2,
                    "type1": type1,
        ...
        if len(unique_possibilities) == 1:
            confidence = 0.9  # Hampir pasti
        elif len(unique_possibilities) <= 3:
            confidence = 0.7  # Beberapa kemungkinan
        else:
            confidence = 0.5  # Banyak kemungkinan
        
        return {
            "type": "partial_two",
            "input_format": f"{part1}-{part2}",
            "interpreted_types": f"{type1}-{type2}",
            "confidence": confidence,
            "num_possibilities": len(unique_possibilities),
            "all_possibilities": unique_possibilities[:10],  # Batasi 10
            "recommendation": self._generate_recommendation(unique_possibilities)
        }
    
    def _find_by_sad_panca(self, sad_norm: str, panca_norm: str, wuku_norm: str = None) -> List[Dict]:
        """Cari triple dengan sadwara dan pancawara tertentu"""
        results = []
        
        for triple_key in self.triple_to_index.keys():
            sad, panca, sapta = triple_key
            
            if sad == sad_norm and panca == panca_norm:
                if wuku_norm:
                    wuku_calc = self.triple_to_wuku.get(triple_key)
                    if wuku_calc != wuku_norm:
                        continue
                ...
                results.append({
                    ...
                    "pancawara": panca,
                    "saptawara": sapta,
                    "wuku": self.triple_to_wuku.get(triple_key),
                    "index": self.triple_to_index.get(triple_key)
                })
        
        return results
    
    def _find_by_sapta_panca(self, sapta_norm: str, panca_norm: str, wuku_norm: str = None) -> List[Dict]:
        """Cari triple dengan saptawara dan pancawara tertentu"""
        results = []
        
        for triple_key in self.triple_to_index.keys():
            sad, panca, sapta = triple_key
            
            if sapta == sapta_norm and panca == panca_norm:
                if wuku_norm:
                    wuku_calc = self.triple_to_wuku.get(triple_key)
                    if wuku_calc != wuku_norm:
                        continue
    ...
    def _find_by_sad_sapta(self, sad_norm: str, sapta_norm: str, wuku_norm: str = None) -> List[Dict]:
        """Cari triple dengan sadwara dan saptawara tertentu"""
        results = []
        
        for triple_key in self.triple_to_index.keys():
            sad, panca, sapta = triple_key
            
            if sad == sad_norm and sapta == sapta_norm:
                if wuku_norm:
                    wuku_calc = self.triple_to_wuku.get(triple_key)
                    if wuku_calc != wuku_norm:
                        continue
    ...
    def _find_by_any_two(self, part1_norm: str, part2_norm: str, wuku_norm: str = None) -> List[Dict]:
        """Cari triple yang mengandung kedua komponen (dalam posisi apapun)"""
        results = []
        
        for triple_key in self.triple_to_index.keys():
            sad, panca, sapta = triple_key
            
            # Cek apakah kedua parts ada dalam triple
            parts_in_triple = [sad, panca, sapta]
            if part1_norm in parts_in_triple and part2_norm in parts_in_triple:
                if wuku_norm:
                    wuku_calc = self.triple_to_wuku.get(triple_key)
                    if wuku_calc != wuku_norm:
                        continue
    ...
    def _parse_single(self, part: str, wuku: str) -> Dict:
        """Parsing single component"""
        part_norm = self.norm.normalize(part)
        wuku_norm = self.norm.normalize(wuku) if wuku else None
        
        # Identifikasi tipe komponen
        comp_type = self.identify_component_type(part)
        
        results = []
        
        # Cari semua triple yang mengandung komponen ini
        for triple_key in self.triple_to_index.keys():
            sad, panca, sapta = triple_key
            
            match = False
            
            if comp_type == "sadwara" and sad == part_norm:
                match = True
            elif comp_type == "pancawara" and panca == part_norm:
                match = True
            elif comp_type == "saptawara" and sapta == part_norm:
                match = True
            elif comp_type == "unknown":
                # Coba semua
                if part_norm in [sad, panca, sapta]:
                    match = True
        ...
        if not results:
            return {
                "error": f"Komponen '{part}' tidak ditemukan dalam sistem",
        ...
        if len(results) == 1:
            confidence = 0.8
        elif len(results) <= 7:  # Maksimal 7 kemungkinan dalam satu wuku
            confidence = 0.6
        else:
            confidence = 0.4
        ...
        return {
            ...
            "component_type": comp_type,
            "confidence": confidence,
            "num_possibilities": len(results),
            "all_possibilities": results[:15],  # Batasi 15
            "note": self._get_single_component_note(comp_type, len(results))
        }
    
    def _generate_recommendation(self, possibilities: List[Dict]) -> str:
        """Buat rekomendasi cerdas berdasarkan hasil parsing"""
        if len(possibilities) == 1:
            poss = possibilities[0]
            return f"✅ Solusi unik: {poss['wuku']} - {poss['sadwara']}-{poss['pancawara']}-{poss['saptawara']}"
        
        # Kelompokkan berdasarkan wuku
        wuku_groups = {}
        for poss in possibilities:
            wuku = poss.get('wuku', 'Unknown')
            if wuku not in wuku_groups:
                wuku_groups[wuku] = []
            wuku_groups[wuku].append(poss)
        
        if len(wuku_groups) == 1:
            # Semua di wuku yang sama
            wuku = list(wuku_groups.keys())[0]
            return f"⚠️  {len(possibilities)} kemungkinan dalam wuku {wuku}"
        else:
            # Tersebar di beberapa wuku
            wuku_list = list(wuku_groups.keys())
            return f"⚠️  {len(possibilities)} kemungkinan tersebar di {len(wuku_list)} wuku"
    
    def _get_single_component_note(self, comp_type: str, num_results: int) -> str:
        """Catatan untuk single component parsing"""
        if comp_type == "unknown":
            return "Komponen tidak dikenali. Mungkin salah ketik atau varian belum terdaftar."
        
        notes = {
            "sadwara": f"Sadwara memiliki 35 kemungkinan kombinasi (7 wuku × 5 pancawara)",
            "pancawara": f"Pancawara memiliki 42 kemungkinan kombinasi (7 wuku × 6 sadwara)",
            "saptawara": f"Saptawara memiliki 30 kemungkinan kombinasi (30 wuku)"
        }
        
        base_note = notes.get(comp_type, "")
        
        if num_results > 10:
            return f"{base_note} Butuh informasi tambahan (wuku/tithi) untuk menyempitkan."
        else:
            return f"{base_note} Ditemukan {num_results} kemungkinan."
...
class ΩSthapatiSystem:
    ...
    def __init__(self, verbose_startup: bool = True):
        ...
        # **TAMBAHKAN SMART WARA PARSER**
        self.smart_wara_parser = SmartWaraParser(self.mech)
        
        # Verifikasi sistem
        self._verify_system_consistency(verbose_startup)
    
    def parse_and_validate_wara(self, wara_string: str, wuku: str = None, verbose: bool = False):
        """
        Parse wara_string dan validasi dengan sistem.
        Return semua kemungkinan triple yang valid.
        """
        if verbose:
            print(f"\n🔍 PARSING WARA: '{wara_string}'")
            if wuku:
                print(f"   Dengan wuku: {wuku}")
        
        result = self.smart_wara_parser.parse_wara_intelligent(wara_string, wuku)
        
        if verbose:
            if "error" in result:
                print(f"   ❌ ERROR: {result['error']}")
            else:
                print(f"   ✅ Berhasil parsing")
                print(f"   Type: {result.get('type')}")
                print(f"   Confidence: {result.get('confidence', 0):.2f}")
                print(f"   Kemungkinan: {result.get('num_possibilities', 0)}")
                
                if "all_possibilities" in result:
                    poss = result["all_possibilities"]
                    if len(poss) <= 3:
                        for i, p in enumerate(poss[:3]):
                            print(f"   {i+1}. {p.get('wuku')}: {p['sadwara']}-{p['pancawara']}-{p['saptawara']}")
        
        return result
    
    # Modifikasi metode convert_prasasti untuk menggunakan parser cerdas
    def convert_prasasti_with_smart_parsing(self, prasasti_data: Dict, verbose: bool = True) -> List[Dict]:
        """
        Konversi data prasasti dengan parsing wara yang cerdas.
        Mendukung semua format wara tidak lengkap.
        """
        if verbose:
            print("\n" + "=" * 80)
            print("Ω-STHAPATI v301.4: SMART WARA PARSING ENABLED")
            print("=" * 80)
            print("Mendukung format wara:")
            print("1. Lengkap: 'Tungleh-Pahing-Aditya'")
            print("2. Parsial: 'Jumat-Wage', 'Haryang-Wage', 'Saniscara-Pahing'")
            print("3. Tunggal: 'Saniscara', 'Wage', 'Haryang'")
            print("=" * 80)
        
        # 1. Parsing wara_string dengan parser cerdas
        wara_string = prasasti_data.get('wara_string', '')
        wuku = prasasti_data.get('wuku', '')
        
        parsing_result = self.parse_and_validate_wara(wara_string, wuku, verbose)
        
        if "error" in parsing_result:
            print(f"❌ Gagal parsing wara: {parsing_result['error']}")
            return []
        
        # 2. Jika ada multiple possibilities, proses semua
        all_candidates = []
        
        if "all_possibilities" in parsing_result:
            possibilities = parsing_result["all_possibilities"]
            num_poss = len(possibilities)
            
            if verbose:
                print(f"\n📋 Ditemukan {num_poss} kemungkinan triple wara:")
                for i, poss in enumerate(possibilities[:5]):  # Tampilkan 5 pertama
                    print(f"   {i+1}. {poss['sadwara']}-{poss['pancawara']}-{poss['saptawara']} (wuku: {poss.get('wuku', '?')})")
                if num_poss > 5:
                    print(f"   ... dan {num_poss-5} kemungkinan lainnya")
            
            # Proses setiap kemungkinan
            for i, poss in enumerate(possibilities):
                if verbose:
                    print(f"\n🔄 Memproses kemungkinan #{i+1}/{num_poss}")
                
                # Buat data prasasti dengan triple lengkap
                prasasti_copy = prasasti_data.copy()
                prasasti_copy['wara_string'] = f"{poss['sadwara']}-{poss['pancawara']}-{poss['saptawara']}"
                
                # Konversi dengan triple lengkap
                results = self.convert_prasasti(prasasti_copy, verbose=False)
                
                for result in results:
                    # Tambahkan info parsing
                    result['wara_parsing_info'] = {
                        'original_input': wara_string,
                        'parsed_triple': poss,
                        'parsing_confidence': parsing_result.get('confidence', 0),
                        'possibility_index': i+1,
                        'total_possibilities': num_poss
                    }
                    all_candidates.append(result)
            
            # Gabungkan dan urutkan
            if all_candidates:
                # Hapus duplikat berdasarkan KA
                unique_candidates = []
                seen_ka = set()
                
                for cand in all_candidates:
                    ka = cand['candidate']['ka']
                    if ka not in seen_ka:
                        seen_ka.add(ka)
                        unique_candidates.append(cand)
                
                # Urutkan berdasarkan skor TPDP
                unique_candidates.sort(key=lambda x: x['score'], reverse=True)
                
                if verbose:
                    print(f"\n✨ HASIL AKHIR: {len(unique_candidates)} kandidat unik")
                    for i, cand in enumerate(unique_candidates[:3]):  # Tampilkan 3 terbaik
                        candidate = cand['candidate']
                        year, month, day = candidate['date']
                        print(f"   #{i+1}: {int(year)}-{int(month):02d}-{int(day):02d} (skor: {cand['score']:.3f})")
                
                return unique_candidates
            else:
                if verbose:
                    print("❌ Tidak ada kandidat yang ditemukan untuk semua kemungkinan triple")
                return []
        
        # Jika parsing langsung menghasilkan triple tunggal
        elif "sadwara" in parsing_result and "pancawara" in parsing_result and "saptawara" in parsing_result:
            # Buat data prasasti dengan triple lengkap
            prasasti_copy = prasasti_data.copy()
            prasasti_copy['wara_string'] = f"{parsing_result['sadwara']}-{parsing_result['pancawara']}-{parsing_result['saptawara']}"
            
            # Konversi seperti biasa
            return self.convert_prasasti(prasasti_copy, verbose)
...
# ============================================================================
# FUNGSI TESTING PARSING WARA CERDAS
# ============================================================================

def test_wara_parsing_intelligence():
    """Test semua kasus wara tidak lengkap"""
    ...
    test_cases = [
        # Format Lengkap
        ("Haryang-Wage-Sukra", None, "Full triple"),
        ("tungleh-pahing-aditya", None, "TU-PA-Ā"),
        
        # Format Parsial (2 parts) - Saptawara-Pancawara
        ("Jumat-Wage", None, "Hari-Pancawara"),
        ("Saniscara-Pahing", None, "Hari-Pancawara"),
        
        # Format Parsial (2 parts) - Sadwara-Pancawara
        ("Haryang-Wage", None, "Sadwara-Pancawara"),
        ("Wurukung-Pon", None, "Sadwara-Pancawara"),
        
        # Format Parsial (2 parts) - Mixed
        ("Haryang-Saniscara", None, "Sadwara-Hari"),
        ("Pon-Sukra", None, "Pancawara-Hari"),
        
        # Single Component
        ("Saniscara", None, "Saptawara saja"),
        ("Wage", None, "Pancawara saja"),
        ("Haryang", None, "Sadwara saja"),
        
        # Dengan Wuku (mempersempit)
        ("Jumat-Wage", "Wayang", "Dengan wuku Wayang"),
        ("Haryang", "Wukir", "Sadwara dengan wuku"),
        
        # Varian ejaan (dari konstanta)
        ("Jumat", None, "Varian: Jumat"),
        ("Sukra", None, "Varian: Sukra"),
        ("Umanis", None, "Varian: Umanis"),
        ("Kaliwon", None, "Varian: Kaliwon"),
    ]
    
    print("🧪 TESTING PARSING WARA TIDAK LENGKAP")
    print("=" * 60)
    
    for wara_string, wuku, description in test_cases:
        print(f"\nTest: {description}")
        print(f"Input: '{wara_string}'" + (f" (wuku: {wuku})" if wuku else ""))
        
        result = system.parse_and_validate_wara(wara_string, wuku, verbose=True)
        
        # PERBAIKAN: Cek dengan aman apakah ada 'num_possibilities' atau 'all_possibilities'
        if "error" in result:
            print(f"   ❌ ERROR: {result['error']}")
            continue
        ...
        num_poss = 0
        if "num_possibilities" in result:
            num_poss = result["num_possibilities"]
        elif "all_possibilities" in result:
            num_poss = len(result["all_possibilities"])
        
        if num_poss == 1:
            print(f"   ✅ SOLUSI UNIK")
        elif num_poss <= 5:
            print(f"   ⚠️  {num_poss} kemungkinan")
        elif num_poss > 5:
            print(f"   ❓ {num_poss} kemungkinan (butuh info tambahan)")
        else:
            print(f"   ❌ Tidak ada kemungkinan")
...
def main():
    ...
    # Tambahkan contoh untuk parsing cerdas
    smart_parsing_samples = [
        {
            "id": "Test_Parsial_Jumat_Wage",
            "saka_year": 851,
            "masa": "Asuji",
            "tithi": 12,
            "paksa": "Sukla",
            "wuku": "Wugu",
            "wara_string": "Jumat-Pahing"  # Format parsial
        },
        {
            "id": "Test_Sadwara_Pancawara", 
            "saka_year": 963,
            "masa": "Kartika",
            "tithi": 10,
            "paksa": "Sukla",
            "wuku": "Wayang",
            "wara_string": "Haryang-Wage"  # Format parsial sadwara-pancawara
        },
        {
            "id": "Test_Single_Saniscara",
            "saka_year": 1057,
            "masa": "Bhadrapada",
            "tithi": 13,
            "paksa": "Krsna",
            "wuku": "Wukir",
            "wara_string": "Saniscara"  # Format tunggal
        }
    ]
    
    # Gabungkan untuk batch processing
    all_sample_inscriptions = sample_inscriptions + smart_parsing_samples
    ...
    while True:
        ...
        print("9. Test Parsing Wara Cerdas")
        print("10. Konversi dengan Parsing Wara Cerdas")
        print("11. Test Smart Parsing Samples")
        print("12. Keluar")
        print("=" * 80)
        ...
        if choice == "1":
            ...
            print("\n" + "=" * 80)
            print("BATCH PROCESSING - ALL SAMPLE INSCRIPTIONS")
            print("=" * 80)
            
            results = system.batch_process_inscriptions(all_sample_inscriptions, verbose=True)
            input("\nTekan Enter untuk kembali...")
            ...
            print("\n" + "=" * 80)
            print("TEST PARSING WARA CERDAS")
            print("=" * 80)
            test_wara_parsing_intelligence()
            input("\nTekan Enter untuk kembali...")
        
        elif choice == "10":
            print("\n" + "=" * 80)
            print("KONVERSI DENGAN PARSING WARA CERDAS")
            print("=" * 80)
            ...
            try:
                ...
                paksa = input("Paksa (Sukla/Krsna): ")
                wuku = input("Wuku (opsional): ")
                wara_string = input("Wara (bisa lengkap/parsial): ")
                nakshatra = input("Nakshatra (opsional): ")
                ...
                results = system.convert_prasasti_with_smart_parsing(prasasti_data, verbose=True)
            ...
            print("\n" + "=" * 80)
            print("TEST SMART PARSING SAMPLES")
            print("=" * 80)
            
            for i, inscription in enumerate(smart_parsing_samples):
                print(f"\n[{i+1}/{len(smart_parsing_samples)}] {inscription['id']}:")
                print(f"   Input: {inscription['wara_string']}")
                
                results = system.convert_prasasti_with_smart_parsing(inscription, verbose=True)
